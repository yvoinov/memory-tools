...
#
# Generate backtrace from a Solaris core dump.
#
# Version: 1.0
...
detect_debugger()
{
  ...
  fail "No supported debugger found (dbx/gdb/lldb)."
}
...
run_debugger()
{
  case "$DEBUGGER" in
    ...
    dbx)
      dbx "$EXECUTABLE" "$ANALYSIS_CORE" <<EOF
where
quit
EOF
      ;;
...
analyze_core()
{
  ...
  case "$DEBUGGER" in
    ...
    dbx)
      grep '^=>' "$OUTPUT_TEMP" >/dev/null 2>&1 || {
        cat "$OUTPUT_TEMP" >&2
        rm -f "$OUTPUT_TEMP"
        OUTPUT_TEMP=""
        fail "DBX did not produce a usable backtrace."
      }
      ;;
    ...
    lldb)
      grep -E '(frame|thread #|stop reason)' "$OUTPUT_TEMP" >/dev/null 2>&1 || {
        cat "$OUTPUT_TEMP" >&2
        rm -f "$OUTPUT_TEMP"
        OUTPUT_TEMP=""
